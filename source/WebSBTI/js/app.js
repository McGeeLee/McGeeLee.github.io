...
const TYPE_LIBRARY = {
  ...
  'ATM-er': { code: 'ATM-er', cn: '送钱者', intro: '你以为我很有钱吗？', desc: '恭喜您，您竟然测出了这个世界上最稀有的的人格。您或将成为金融界的未解之谜——是的，ATM-er不一定真的"送钱"，但可能永远在"支付"。支付时间、支付精力、支付耐心、支付一个本该安宁的夜晚。因此像一部老旧但坚固的ATM机，插进去的是别人的焦虑和麻烦，吐出来的是"没事，有我"的安心保证。您的人生就是一场盛大的、无人喝彩的单人付账秀。您竟用磐石般的可靠，承受了瀑布般的索取，偶尔夜深人静才会对着账单——可能是精神上的——发出一声叹息：我这该死的、无处安放的责任心啊。' },
  ...
  DEAD: { code: 'DEAD', cn: '死者', intro: '我没死，我只是先行一步。', desc: '恭喜您，您测出了全宇宙最寂寥的人格——DEAD。在人类文明的长河里，有一类人如同深海生物，遥远、沉默、几乎不被察觉。他们不是不存在，只是选择了与这个世界保持一种奇异的距离。当所有人都在热烈地发朋友圈、发微博、发抖音，展示自己生活的每个高光时刻时，DEAD们安静地滑动屏幕，给每一个帖子里的人默默点了个赞，然后继续潜入更深的海底。' },
  IMFW: { code: 'IMFW', cn: '免疫者', intro: '无所谓，我会出手。', desc: '恭喜您！您测出了宇宙间最后的净土人格——IMFW。这是一种无敌的免疫状态。当代年轻人容易被社会时钟绑架、容易被他人评价动摇、容易被算法推送操控、容易被消费主义诱惑的时代里，IMFW群体像一群安装了金钟罩的修行者。他们免疫年龄焦虑，免疫成功学PUA，免疫他人的指指点点，免疫一切形式的自我内耗。他们的存在本身就是对快节奏社会的一次优雅反抗。' },
  DRUNK: { code: 'DRUNK', cn: '酒神', intro: '醒着有什么意思？', desc: '恭喜您激活了隐藏人格——DRUNK。酒精异常因子已接管您的系统。在这个世界里，规矩是拿来打破的，限制是拿来超越的，沉默是拿来打破的。DRUNK人格的特点是：颠覆秩序，即兴发挥，永不宕机。他们不是在喝酒，他们是在进行一次又一次的哲学实验——通过酒精来探索意识的边界、释放被压抑的创造力。对DRUNK来说，酒后吐真言不是一句调侃，而是一种生活方式。' },
  HHHH: { code: 'HHHH', cn: '兜底者', intro: '系统强制兜底，请接受现实。', desc: '您的测试结果触发了系统强制兜底机制。这意味着您的答题模式与标准人格库中的任何一种都有太大差异。这是好事！HHHH人格是为您这样的人准备的——那些不愿意被任何标签定义的人。那些在人群中独树一帜，拒绝被归类的人。' }
};

const TYPE_IMAGES = {
  CTRL: './images/sbti_images/CTRL.png',
  BOSS: './images/sbti_images/BOSS.png',
  'ATM-er': './images/sbti_images/ATM-er.png',
  'Dior-s': './images/sbti_images/Dior-s.jpg',
  'THAN-K': './images/sbti_images/THAN-K.png',
  GOGO: './images/sbti_images/GOGO.png',
  SEXY: './images/sbti_images/SEXY.png',
  OJBK: './images/sbti_images/OJBK.png',
  'JOKE-R': './images/sbti_images/JOKE-R.jpg',
  POOR: './images/sbti_images/POOR.png',
  'OH-NO': './images/sbti_images/OH-NO.png',
  MONK: './images/sbti_images/MONK.png',
  SHIT: './images/sbti_images/SHIT.png',
  MALO: './images/sbti_images/MALO.png',
  'THIN-K': './images/sbti_images/THIN-K.png',
  SOLO: './images/sbti_images/SOLO.png',
  'LOVE-R': './images/sbti_images/LOVE-R.png',
  'WOC!': './images/sbti_images/WOC.png',
  DRUNK: './images/sbti_images/DRUNK.png',
  IMFW: './images/sbti_images/IMFW.png',
  'MUM': './images/sbti_images/MUM.png',
  'FAKE': './images/sbti_images/FAKE.png',
  IMSB: './images/sbti_images/IMSB.png',
  FUCK: './images/sbti_images/FUCK.png',
  DEAD: './images/sbti_images/DEAD.png',
  HHHH: './images/sbti_images/HHHH.png'
};
...
function computeResult(answers) {
  ...
  Object.entries(rawScores).forEach(entry => {
    const dim = entry[0];
    const score = entry[1];
    levels[dim] = score <= 3 ? 'L' : score === 4 ? 'M' : 'H';
  });

  const finalLevels = dimensionOrder.map(dim => letterToNumber(levels[dim]));

  const rankedTypes = NORMAL_TYPES.map(type => {
    const patternArray = type.pattern.replace(/-/g, '').split('').map(letterToNumber);
    let distance = 0;
    let exact = 0;

    for (let i = 0; i < patternArray.length; i++) {
      const diff = Math.abs(finalLevels[i] - patternArray[i]);
      distance += diff;
      if (diff === 0) exact += 1;
    }

    const similarity = Math.max(0, Math.round(100 * (1 - distance / 30)));
  ...
  let bestNormal = rankedTypes[0];
  let isDrunk = answers.drink_gate_q2 === 2;
  ...
  const dimData = dimensionOrder.map(dim => ({
  ...
  let posterImage = TYPE_IMAGES[finalType.code] || '';
  if (posterImage) {
    posterImage = posterImage.replace('./image/', '/images/sbti_images/');
  }
  ...
  return {
    ...
    posterImage,
    dimData,
    funNote: special ? '本测试仅供娱乐。隐藏人格和傻乐兜底都属于作者故意埋的损招，请勿把它当成医学、心理学、相学、命理学或灵异学依据。' : '本测试仅供娱乐，别拿它当诊断、面试、相亲、分手、招魂、算命或人生判决书。你可以笑，但别太当真。'
  };
}
...
function saveAnswers(answers) {
  localStorage.setItem('sbti_answers', JSON.stringify(answers));
}

function getAnswers() {
  const stored = localStorage.getItem('sbti_answers');
  return stored ? JSON.parse(stored) : {};
}

function saveResult(result) {
  localStorage.setItem('sbti_result', JSON.stringify(result));
}

function getResult() {
  const stored = localStorage.getItem('sbti_result');
  return stored ? JSON.parse(stored) : null;
}

function clearResult() {
  localStorage.removeItem('sbti_answers');
  localStorage.removeItem('sbti_result');
}